#!/usr/bin/env python
"""
train_efficientdet_final.py
============================
Entrenamiento robusto de EfficientDet-D2 para detección de placas vehiculares ecuatorianas.

Características:
  - Nunca crashea: manejo exhaustivo de errores en cada paso crítico
  - Pause/Resume automático desde último checkpoint guardado
  - Warmup de backbone (5 épocas congelado → descongelado gradual)
  - Métricas finales reales: mAP@0.5, mAP@0.5:0.95, Precision, Recall, F1
  - Export ONNX automático al terminar con benchmark de latencia
  - Historial completo en JSON + resumen en consola

Uso:
  py train_efficientdet_final.py                  → entrena combined_all (recomendado)
  py train_efficientdet_final.py combined_all     → global + ec-1 + ec-2 + ec-4
  py train_efficientdet_final.py combined         → ec-1 + ec-2 + ec-4
  py train_efficientdet_final.py ecuador4         → solo ec-4 (debug rápido)
  py train_efficientdet_final.py export           → solo exportar ONNX desde best.pt
...
NUM_CLASSES   = 1       # 1 clase = 'placa' (EfficientDet internamente maneja background=0)
PATIENCE      = 20
WARMUP_EPOCHS = 5
GRAD_CLIP     = 1.0

# Normalización ImageNet (backbone preentrenado)
NORM_MEAN = [0.485, 0.456, 0.406]
NORM_STD  = [0.229, 0.224, 0.225]

# Umbrales para métricas finales
IOU_THRESHOLD   = 0.5
CONF_THRESHOLD  = 0.25
...
class PlateDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # ── Cargar labels YOLO → pascal_voc (x1,y1,x2,y2 en píxeles) ──────────
        boxes, labels = [], []
                ...
                with open(lbl_path) as f:
                    for line in f:
                        ...
                        if x2 - x1 >= 2 and y2 - y1 >= 2:
                            ...
                            # CRÍTICO: effdet usa 0 para background, los objetos deben ser >= 1
        ...
        # ── Conversión pascal_voc → EfficientDet [y1,x1,y2,x2] ───────────────
    ...
    @staticmethod
    def _dummy():
        """Item vacío válido para sustituir imágenes corruptas o sin objetos."""
    ...
    for imgs, targets in pbar:
        try:
            ...
            if "out of memory" in str(e).lower():
                print(f"\n  [WARN] OOM en batch — liberando memoria y continuando")
                torch.cuda.empty_cache()
            else:
                print(f"\n  [WARN] Error en batch de train: {e}")
            continue
...
def compute_metrics(
    ...
    usando el mejor modelo guardado.
    """
    ...
    for imgs, targets in tqdm(val_loader, desc="  Eval ", dynamic_ncols=True, leave=False):
        ...
        try:
            img_scales = torch.ones(imgs.shape[0], dtype=torch.float32, device=device)
            ...
            for t in targets:
                ...
                valid = (gt_cls > 0) & ((gt_boxes[:, 2] - gt_boxes[:, 0]) >= 2) & ((gt_boxes[:, 3] - gt_boxes[:, 1]) >= 2)
    ...
    def compute_ap_at_iou(preds, gts, iou_thr) -> Tuple[float, float, float]:
        ...
        for thr in np.linspace(0, 1, 101):
            prec = precision_curve[recall_curve >= thr]
            ap  += (np.max(prec) if len(prec) > 0 else 0.0) / 101.0
    ...
    for thr in iou_thresholds:
        ap, _, _ = compute_ap_at_iou(all_preds, all_gts, thr)
        aps.append(ap)

    ap50, prec50, rec50 = compute_ap_at_iou(all_preds, all_gts, 0.5)
...
def train(dataset_key: str) -> str:
    ...
    # Mantenemos el mismo optimizador durante todo el proceso para no perder sus momentos
    ...
    if last_path.exists():
        ...
        try:
            ...
            if start_epoch >= WARMUP_EPOCHS:
                ...
                steps = start_epoch - WARMUP_EPOCHS
            else:
                steps = start_epoch
    ...
    for epoch in range(start_epoch, EPOCHS):
        if epoch == WARMUP_EPOCHS and not backbone_thawed:
            ...
            # Solo reducimos el LR pero conservamos el objeto optimizador (para retener los momentos de AdamW)
            for param_group in optimizer.param_groups:
                param_group['lr'] = LR / 5
